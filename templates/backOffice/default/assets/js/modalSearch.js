...
$(document).ready(function(){
    ...
    setModalSearch = function (modalClass) {
        $modalSearch = $('.' + modalClass);
        ...
        $modalSearch.on('click', '.js-modal-btn-select', function(e){
            ...
            if(selectedVal != null) {
                $($modalSearch.data('selectselector')).val(selectedVal);
                var evt = document.createEvent("HTMLEvents");
                evt.initEvent("change", false, true);
                document.getElementById(($modalSearch.data('selectselector')).substring(1)).dispatchEvent(evt);
                $modalSearch.dismiss();
            }
        });
